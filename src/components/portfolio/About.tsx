...
function StatCard({ value, suffix, label, active }: { value: number; suffix: string; label: string; active: boolean }) {
  ...
  return (
    <div className="glass rounded-[1.5rem] border border-white/10 bg-gradient-to-br from-white/10 to-white/5 p-6 text-center shadow-[0_20px_55px_-24px_rgba(0,0,0,0.7)] transition-all duration-300 hover:-translate-y-1 hover:border-white/20">
...
function ValueTag({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="glass rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 to-transparent px-4 py-3 flex items-center gap-2 text-sm text-foreground shadow-[0_10px_30px_-20px_rgba(0,0,0,0.75)]">
...
export function SectionHeading({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle?: string }) {
  return (
    <div className="max-w-3xl">
      <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-brand/10 px-3 py-1.5 text-[11px] uppercase tracking-[0.3em] text-brand-2">
        <Sparkles className="h-3 w-3 text-brand" />
        {eyebrow}
      </div>
      <h2 className="mt-3 text-3xl sm:text-4xl lg:text-5xl font-semibold font-display leading-tight tracking-[-0.02em]">
        {title}
      </h2>
      {subtitle && <p className="mt-3 text-muted-foreground max-w-2xl text-base leading-relaxed">{subtitle}</p>}
    </div>
  );
}
